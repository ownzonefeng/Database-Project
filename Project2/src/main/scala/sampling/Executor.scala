...
object Executor {
  def execute_Q1(desc: Description, session: SparkSession, params: List[Any]) = {
    ...
    val queryLang: String = s"select\n\tl_returnflag,\n\tl_linestatus,\n\tsum(l_quantity) * ${desc.sampleDescription.head._1} as sum_qty,\n\tsum(l_extendedprice) * ${desc.sampleDescription.head._1} as sum_base_price,\n\tsum(l_extendedprice * (1 - l_discount)) * ${desc.sampleDescription.head._1} as sum_disc_price,\n\tsum(l_extendedprice * (1 - l_discount) * (1 + l_tax)) * ${desc.sampleDescription.head._1} as sum_charge,\n\tavg(l_quantity) as avg_qty,\n\tavg(l_extendedprice) as avg_price,\n\tavg(l_discount) as avg_disc,\n\tcount(*) * ${desc.sampleDescription.head._1} as count_order\nfrom\n\tlineitem\nwhere\n\tl_shipdate <= date '1998-12-01' - interval ${params.head} day \ngroup by\n\tl_returnflag,\n\tl_linestatus\norder by\n\tl_returnflag,\n\tl_linestatus"
  ...
  def execute_Q5(desc: Description, session: SparkSession, params: List[Any]) = {
    //
    val queryLang: String = s"select\n\tn_name,\n\tsum(l_extendedprice * (1 - l_discount)) * ${desc.sampleDescription(1)._1} as revenue\nfrom\n\tcustomer,\n\torders,\n\tlineitem,\n\tsupplier,\n\tnation,\n\tregion\nwhere\n\tc_custkey = o_custkey\n\tand l_orderkey = o_orderkey\n\tand l_suppkey = s_suppkey\n\tand c_nationkey = s_nationkey\n\tand s_nationkey = n_nationkey\n\tand n_regionkey = r_regionkey\n\tand r_name = '${params(0)}'\n\tand o_orderdate >= date '${params(1)}'\n\tand o_orderdate < date '${params(1)}' + interval '1' year\ngroup by\n\tn_name\norder by\n\trevenue desc"
    val test: StructType = desc.lineitem.schema
    val lineitem: DataFrame = session.createDataFrame(desc.samples(1), test)
    lineitem.createOrReplaceTempView("lineitem")
    desc.customer.createOrReplaceTempView("customer")
    desc.orders.createOrReplaceTempView("orders")
  ...
  def execute_Q7(desc: Description, session: SparkSession, params: List[Any]) = {
    //
    val queryLang1: String = s"select\n\t\t\tn1.n_name as supp_nation,\n\t\t\tn2.n_name as cust_nation,\n\t\t\tyear(l_shipdate) as l_year,\n\t\t\tl_extendedprice * (1 - l_discount) as volume\n\t\tfrom\n\t\t\tsupplier,\n\t\t\tlineitem,\n\t\t\torders,\n\t\t\tcustomer,\n\t\t\tnation n1,\n\t\t\tnation n2\n\t\twhere\n\t\t\ts_suppkey = l_suppkey\n\t\t\tand o_orderkey = l_orderkey\n\t\t\tand c_custkey = o_custkey\n\t\t\tand s_nationkey = n1.n_nationkey\n\t\t\tand c_nationkey = n2.n_nationkey\n\t\t\tand (\n\t\t\t\t(n1.n_name = '${params(0)}' and n2.n_name = '${params(1)}')\n\t\t\t\tor (n1.n_name = '${params(1)}' and n2.n_name = '${params(0)}')\n\t\t\t)"
    val queryLang2: String = s"select\n\tsupp_nation,\n\tcust_nation,\n\tl_year,\n\tsum(volume) * ${desc.sampleDescription.head._1} as revenue\nfrom tempView group by\n\tsupp_nation,\n\tcust_nation,\n\tl_year\norder by\n\tsupp_nation,\n\tcust_nation,\n\tl_year"
    val test: StructType = desc.lineitem.schema
    val lineitem: DataFrame = session.createDataFrame(desc.samples.head, test)
    lineitem.createOrReplaceTempView("lineitem")
    desc.customer.createOrReplaceTempView("customer")
    ...
    desc.supplier.createOrReplaceTempView("supplier")
    desc.nation.createOrReplaceTempView("nation")
    session.sql(queryLang1).createOrReplaceTempView("tempView")
    session.sql(queryLang2).show()
  }

  def execute_Q9(desc: Description, session: SparkSession, params: List[Any]) = {
    //
    val queryLang: String = s"select\n\tnation,\n\to_year,\n\tsum(amount) * ${desc.sampleDescription(1)._1} as sum_profit\nfrom\n\t(\n\t\tselect\n\t\t\tn_name as nation,\n\t\t\tyear(o_orderdate) as o_year,\n\t\t\tl_extendedprice * (1 - l_discount) - ps_supplycost * l_quantity as amount\n\t\tfrom\n\t\t\tpart,\n\t\t\tsupplier,\n\t\t\tlineitem,\n\t\t\tpartsupp,\n\t\t\torders,\n\t\t\tnation\n\t\twhere\n\t\t\ts_suppkey = l_suppkey\n\t\t\tand ps_suppkey = l_suppkey\n\t\t\tand ps_partkey = l_partkey\n\t\t\tand p_partkey = l_partkey\n\t\t\tand o_orderkey = l_orderkey\n\t\t\tand s_nationkey = n_nationkey\n\t\t\tand p_name like '%${params(0)}%'\n\t) as profit\ngroup by\n\tnation,\n\to_year\norder by\n\tnation,\n\to_year desc"
    val test: StructType = desc.lineitem.schema
    val lineitem: DataFrame = session.createDataFrame(desc.samples(1), test)
    lineitem.createOrReplaceTempView("lineitem")
  ...
  def execute_Q17(desc: Description, session: SparkSession, params: List[Any]) = {
    //
    val queryLang: String = s"select\n\tsum(l_extendedprice) * ${desc.sampleDescription(2)._1} / 7.0 as avg_yearly\nfrom\n\tlineitem,\n\tpart\nwhere\n\tp_partkey = l_partkey\n\tand p_brand = '${params(0)}'\n\tand p_container = '${params(1)}'\n\tand l_quantity < (\n\t\tselect\n\t\t\t0.2 * avg(l_quantity)\n\t\tfrom\n\t\t\tlineitem\n\t\twhere\n\t\t\tl_partkey = p_partkey\n\t)"
    val test: StructType = desc.lineitem.schema
    val lineitem: DataFrame = session.createDataFrame(desc.samples(2), test)
    lineitem.createOrReplaceTempView("lineitem")
    desc.part.createOrReplaceTempView("part")
  ...
  def execute_Q18(desc: Description, session: SparkSession, params: List[Any]) = {
    //
    val queryLang: String = s"select\n\tc_name,\n\tc_custkey,\n\to_orderkey,\n\to_orderdate,\n\to_totalprice,\n\tsum(l_quantity)* ${desc.sampleDescription(2)._1}\nfrom\n\tcustomer,\n\torders,\n\tlineitem\nwhere\n\to_orderkey in (\n\t\tselect\n\t\t\tl_orderkey\n\t\tfrom\n\t\t\tlineitem\n\t\tgroup by\n\t\t\tl_orderkey having\n\t\t\t\tsum(l_quantity) > ${params(0)}\n\t)\n\tand c_custkey = o_custkey\n\tand o_orderkey = l_orderkey\ngroup by\n\tc_name,\n\tc_custkey,\n\to_orderkey,\n\to_orderdate,\n\to_totalprice\norder by\n\to_totalprice desc,\n\to_orderdate"
    val test: StructType = desc.lineitem.schema
    val lineitem: DataFrame = session.createDataFrame(desc.samples(2), test)
    lineitem.createOrReplaceTempView("lineitem")
    desc.part.createOrReplaceTempView("part")
